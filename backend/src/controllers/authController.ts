    ...
    try {
      const userRes = await pool.query(`
        SELECT u.id, u.nome, u.email, u.matricula, u.senha_hash, u.perfil, 
               ...
               u.ativo,
               COALESCE(u.tentativas_login_falhas, 0) AS tentativas_login_falhas,
               u.bloqueado_ate,
      ...
      // Senha correta: resetar tentativas falhas e bloqueio
      await pool.query(
        'UPDATE usuarios SET tentativas_login_falhas = 0, bloqueado_ate = NULL WHERE id = $1',
        [user.id]
      );

      // Checar se o usuário possui 2FA TOTP (Google Authenticator) ativado
      if (user.totp_ativo && user.totp_secret) {
